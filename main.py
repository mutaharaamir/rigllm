    ...
    for layer in layers:
        ...
        for name, module in layer.named_modules():
            if isinstance(module, nn.Linear):
                ...
                mask = (rand_scores > threshold).bool()

                module.register_buffer('mask', mask)
        layer.to('cpu')
        torch.cuda.empty_cache()

def update_mask(layer, drop_fraction, mask_update_idx):
    for name, module in layer.named_modules():
        ...
        # logging
        wandb.log({
            'mask_update_step': mask_update_idx,
            f'layer_sparsity/{name}': actual_sparsity,
        })
...
def prune_rigl(model, tokenizer, device='cuda', sparsity=0.5, nsamples=128,
    ...
    # print(f'Mask Initialized. {time.perf_counter() - start:.2f}s')
    print("Mask Initialized. Starting...")
    ...
    current_inps = inps
    mask_update_idx = 0 # todo: remove maybe?
    for layer_idx, layer in enumerate(layers):
        ...
        for i in range(num_epochs):
            ...
            # TODO: batch these? doesnt have to be a for-loop if enough vram tbh
            for idx, inp in enumerate(current_inps):
                ...
                if (idx+1) % update_schedule == 0:
                    print('updating mask')
                    mask_update_idx += 1
                    update_mask(layer, drop_fraction=drop_fraction, mask_update_idx=mask_update_idx) # todo: pass in layer number?
            ...
            global_epoch = (layer_idx * num_epochs) + (i + 1)
            ...
            wandb.log({
                ...
                'sequential_loss': epoch_loss,

                f'layer_comparison/loss_layer_{layer_idx+1}': epoch_loss,
            })
        ...
        wandb.log({ #todo: fix
            f"layer_summaries/initial_loss": initial_layer_loss,
            f"layer_summaries/final_loss": final_layer_loss,
            f"layer_summaries/delta_improvement": delta_loss,
            "layer_summaries/layer_index": layer_idx + 1,
        }, step=layer_idx + 1)
...
def train():
    run = wandb.init(group="update-schedule-sweep")

    nsamples = 128
    sparsity = 0.5
    seed = 42
    seqlen = 2048
    update_schedule = wandb.config.update_schedule
    num_epochs = wandb.config.num_epochs  # how many times we see the whole batch
    drop_fraction = wandb.config.drop_fraction # the paper suggests 0.1/0.3/0.5
    model_name = "meta-llama/Llama-3.2-1B"

    run.name = f"{model_name[-2:]}-{drop_fraction}-{num_epochs}-{sparsity*100}%-{update_schedule}"
    run.notes = f"Sparsity: {sparsity} | Drop Frac: {drop_fraction} | Schedule: {update_schedule} | Epochs: {num_epochs} | model: {model_name}"
    ...
    # initializing mask
    init_mask(model=model, sparsity=sparsity, seed=seed)
    ...
    # 4: calculating final perplexity
    print("Evaluating final RigL pruned perplexity...")
    model.to('cuda')
    final_ppl = eval_ppl(seed, model, tokenizer)
    print(f"Final Optimized PPL: {final_ppl}")
    wandb.log({"ppl/3_final_rigl_pruned": final_ppl})
...
sweep_configuration = {
    'method': 'grid',
    'metric': {'goal': 'minimize', 'name': 'ppl/perplexity after pruning'},
    'parameters': {
        'drop_fraction': {'values': [0.3, 0.5]},
        'num_epochs':{'values': [10, 15]},
        'update_schedule': {'values': [32, 64, 128]}
    }
}

# 3. Initialize and run everything immediately
sweep_id = wandb.sweep(sweep=sweep_configuration, project="rigl-llm")
